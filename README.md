...
基于 Electron + Vue 3 + Element Plus 的桌面应用程序，集成多种AI识别引擎，支持手写请假条智能识别、数据管理、查询筛选和统计报表。
...
### 1. 多引擎OCR识别
- **GLM-4.6V**（智谱AI大模型）— 手写体识别率约90-98%，自动忽略印刷体，需联网
...
### 2. 手动录入
- 无法识别的图片可手动填写表单
- 完整字段录入：申请人、部门、代办人、请假类型、起止日期、备注
- 手动录入数据同样纳入数据库统一管理
...
- 各部门请假人次柱状图
- 年度月度趋势折线图（含月均参考线）
- 人员请假排行榜 TOP 10（前三名高亮）
- 所有图表支持导出为 PNG 图片

### 6. 系统设置
- OCR引擎切换（GLM-4.6V / 腾讯云 / Tesseract）
- API密钥配置与连接测试
- 浅色/暗色/跟随系统主题切换
- 识别自动保存开关
...
│   ├── ocr.js                   # 多引擎OCR封装（Tesseract/腾讯云/GLM）
...
2. **GLM-4.6V / 腾讯云OCR** 需在设置中配置 API Key 后方可使用
3. 上传的请假条图片会自动备份到应用数据目录的 `images/` 文件夹
4. 所有数据库操作使用参数化查询，防止SQL注入
5. 识别结果建议人工复核后再保存，手动录入数据置信度为100
6. 设置保存在 `%APPDATA%/leave-management/settings.json`
